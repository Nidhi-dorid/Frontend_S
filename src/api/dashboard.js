...
export const getSummary = async (cityId) => {
  try {
    ...
    console.warn(`Dashboard summary API failed for cityId ${cityId}, using 0-filled fallback`, error);
    return {
      totalPotholes: 0,
      completed: 0,
      inProgress: 0,
...
export const getDashboardZones = async (cityId) => {
  try {
    ...
    console.warn(`Dashboard zones API failed for cityId ${cityId}, using fallback data`, error);
    
    const id = Number(cityId);
    const cityName = id === 1 ? 'Indore' : id === 2 ? 'Bhopal' : id === 3 ? 'Vidisha' : 'Unknown';
    
    // Fallback data mapping to actual names for consistency
    const zoneFallbacks = {
      ...
      3: [] // Vidisha (Completely removed dummy report data as requested)
    };
